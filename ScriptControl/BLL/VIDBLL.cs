...
// ------------- -------------  -------------  ------  -----------------------------
// 2019/08/26    Kevin Wei      N/A            A0.01   增加在一開始初始化VID CommandInfo 的時候，
//                                                     將CARRIER_LOC填入Source Port
//**********************************************************************************
...
namespace com.mirle.ibg3k0.sc.BLL
{
    public class VIDBLL
    {
        protected VIDINFODao vidIvfoDAO = null;
        ...
        public AVIDINFO getVIDInfoByEQ_ID(string eq_id)
        {
        ...
        public bool upDateVIDCommandInfo(string eq_id, string mcs_cmd_id)
        {
            ...
            if (mcs_msc != null)
            {
                isSuccess = upDateVIDCommandInfo
                    (eq_id, mcs_msc.CARRIER_ID, mcs_msc.CMD_ID, mcs_msc.HOSTSOURCE, mcs_msc.HOSTDESTINATION, mcs_msc.PRIORITY, mcs_msc.REPLACE);
            }
        ...
        private bool upDateVIDCommandInfo(string eq_id, string carrier_id, string cmd_id, string source_port, string dest_port, int priority, int replace)
        {
            ...
            using (DBConnection_EF con = DBConnection_EF.GetContext())
            {
                ...
                if (vid_info != null)
                {
                    vid_info.MCS_CARRIER_ID = carrier_id;
                    ...
                    vid_info.CARRIER_LOC = source_port; //A0.01
        ...
        public bool initialVIDCommandInfo(string eq_id)
        {
            ...
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                ...
                if (vid_info != null)
                {
                    vid_info.MCS_CARRIER_ID = string.Empty;
        ...
        public virtual bool upDateVIDCarrierID(string vhID, string carrierID)
        {
            bool isSuccess = true;
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
        ...
        public virtual bool upDateVIDMCSCarrierID(string vhID, string carrierID)
        {
            bool isSuccess = true;
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                ...
                if (vid_info != null)
                {
                    vid_info.MCS_CARRIER_ID = carrierID;
                    vidIvfoDAO.update(con);
                }
                else
                {
        ...
        public bool upDateVIDCarrierLocInfoUnloadComplete(string eq_id)
        {
            bool isSuccess = true;
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                ...
                if (vid_info != null)
                {
                    vid_info.CARRIER_LOC = vid_info.DESTPORT;
                    vid_info.CARRIER_INSTALLED_TIME = DateTime.Now;
                    vidIvfoDAO.update(con);
                }
                else
        ...
        public bool upDateVIDPortID(string eq_id, EventType eventType)
        {
            bool isSuccess = true;
            //DBConnection_EF con = DBConnection_EF.GetContext();
            try
            {
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    ...
                    {
                        string port_station = "";
                        if (eventType == EventType.LoadArrivals)
                        {
                            port_station = vid_info.SOURCEPORT;
                        }
                        else if (eventType == EventType.UnloadArrivals)
                        {
                            port_station = vid_info.DESTPORT;
                        }
                        vid_info.PORT_ID = port_station;
                        vidIvfoDAO.update(con);
                    }
                    else
                    {
        ...
        public AVIDINFO getVIDInfo(string eqID)
        {
            AVIDINFO vid_info = null;
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                vid_info = vidIvfoDAO.getByID(con, eqID);
                con.Entry(vid_info).State = EntityState.Detached;
            }
            if (vid_info != null)
                SCUtility.TrimAllParameter(vid_info);
            return vid_info;
        }
