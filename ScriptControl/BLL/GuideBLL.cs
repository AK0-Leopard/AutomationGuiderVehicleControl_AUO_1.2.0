...
namespace com.mirle.ibg3k0.sc.BLL
{
    public class GuideBLL
    {
        ...
        getGuideInfoIgnoreCost(string startAddress, string targetAddress, List<string> byPassSectionIDs = null)
        {
            ...
            if (byPassSectionIDs == null || byPassSectionIDs.Count == 0)
            {
                stratFromRouteInfoList = scApp.RouteGuideIgnoreCost.getFromToRoutesAddrToAddr(i_start_address, i_target_address);
            }
            else
            {
                stratFromRouteInfoList = scApp.RouteGuideIgnoreCost.getFromToRoutesAddrToAddr(i_start_address, i_target_address, byPassSectionIDs);
            }
            RouteInfo min_stratFromRouteInfo = null;
            if (stratFromRouteInfoList != null && stratFromRouteInfoList.Count > 0)
            {
        ...
        public virtual void initialUnbanRoute()
        {
            ...
            scApp.RouteGuideIgnoreCost.resetBanRoute();
            ...
            foreach (ASEGMENT seg in segments)
            {
                if (seg.STATUS == E_SEG_STATUS.Closed)
                {
                    foreach (var sec in seg.Sections)
                    {
                        ...
                        scApp.RouteGuideIgnoreCost.banRouteTwoDirect(sec.SEC_ID);//由於目前AGV的圖資資料Section=Segment，之後會將牠們分開，
                                                                               //因此先將Segment作為Sectiong使用
                    }
                }
            }
        ...
        public virtual ASEGMENT unbanRouteTwoDirect(string segmentID)
        {
            ...
            if (segment_vo != null)
            {
                foreach (var sec in segment_vo.Sections)
                {
                    ...
                    scApp.RouteGuideIgnoreCost.unbanRouteTwoDirect(sec.SEC_ID);
                }
            }
        ...
        public virtual ASEGMENT banRouteTwoDirect(string segmentID)
        {
            ...
            if (segment_vo != null)
            {
                foreach (var sec in segment_vo.Sections)
                {
                    ...
                    scApp.RouteGuideIgnoreCost.banRouteTwoDirect(sec.SEC_ID);
                }
            }
    ...
    public class GuldeBLLForFloy : GuideBLL
    {
        ...
        public override ASEGMENT unbanRouteTwoDirect(string segmentID)
        {
            ...
            scApp.RouteGuideIgnoreCost.unbanRouteTwoDirect(segmentID);
        ...
        public override ASEGMENT banRouteTwoDirect(string segmentID)
        {
            ...
            scApp.RouteGuideIgnoreCost.banRouteTwoDirect(segmentID);
            segment = scApp.MapBLL.DisableSegment(segmentID);
            return segment;
        }
        ...
        public override void initialUnbanRoute()
        {
            ...
            foreach (ASEGMENT seg in segments)
            {
                if (seg.STATUS == E_SEG_STATUS.Closed)
                {
                    ...
                    scApp.RouteGuideIgnoreCost.banRouteTwoDirect(seg.SEG_ID);//由於目前AGV的圖資資料Section=Segment，之後會將牠們分開，
                }
            }
        }
